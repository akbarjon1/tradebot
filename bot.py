...
def save_trade_to_sheets(title, content):
    ...
    try:
        ...
        sent_trade_ids.add(str(t_id).strip())  # Saqlangandan so'ng keshga oladi
...
# --- 7. GOOGLE SHEETS MONITORING (XATOLIK TO'G'RILANDI) ---
sent_trade_ids = set()

def monitor_new_trades():
    global sent_trade_ids
    # Dastlabki mavjud eski bitimlarni bazadan keshga olib olamiz (qayta yubormasligi uchun)
    try:
...
# --- 8. AVTOMATIK YANGILIKLAR TIZIMI (XATOLIK TO'G'RILANDI) ---
SEEN_NEWS = set()

def fetch_and_post_crypto_news():
    global SEEN_NEWS
    # Server qayta yonganda eski yangiliklar qayta ketmasligi uchun birinchi 10 tasini keshga olib olamiz
    try:
        init_feed = feedparser.parse("https://cointelegraph.com/rss")
        for entry in init_feed.entries[:10]:
            if entry.get("link"):
                SEEN_NEWS.add(entry.get("link"))
    except Exception as e:
        print(f"Boshlang'ich RSS keshida ogohlantirish: {e}")
